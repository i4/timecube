...
export default class SumBarPanel extends Vue {
	private options: any = {};
	...
	dateChanged(dateRange: DateRange, series: Series[]) {
		this.options = this.generateBar(series, dateRange.start, dateRange.end, {
			chart: {
				height: 350,
				type: "bar",
				toolbar: {
			...
			tooltip: {
				y: {
					formatter: this.formatHourMinute
				}
			}
		});
	}
	...
	accumulateSeries(s: Series, start: Moment, end: Moment) {
		...
		for (var j = 0; j < s.data.length; j++) {
			...
			if (start.unix() <= entry[1] && end.unix() >= entry[0]) {
				sum +=
					Math.min(entry[1], end.unix()) - Math.max(entry[0], start.unix());
			}
		}
	...
	generateBar(series: Series[], start: Moment, end: Moment, options = {}) {
		options.colors = [];
		options.series = [
			{
